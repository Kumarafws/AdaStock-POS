<x-layouts.app title="Edit Produk">
    ...
    <x-slot:subtitle>
        Perbarui informasi produk, harga jual dasar, atau struktur kemasan bertingkat.
    </x-slot:subtitle>

    <div x-data="{
        baseUnit: '{{ $product->base_unit_name }}',
        baseSellingPrice: {{ $product->default_selling_price }},
        units: {{ Js::from($product->units->map(fn($u) => [
        ...
        <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <x-card title="1. Identitas Produk & Kategori" subtitle="Informasi utama untuk mengenali produk di sistem.">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="sku" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Kode SKU *</label>
                        <input type="text" 
                               id="sku" 
                               name="sku" 
                    ...
                    <div class="md:col-span-2">
                        <label for="image" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Ganti Foto Produk (Opsional)</label>
                        <div class="flex items-center gap-4">
                            @if($product->image_path)
                                <img src="{{ asset('storage/' . $product->image_path) }}" alt="{{ $product->name }}" class="w-12 h-12 rounded-xl object-cover border border-slate-200">
                            @endif
                            <input type="file" 
                                   id="image" 
                                   name="image" 
            ...
            <x-card title="2. Satuan Dasar (Base Unit) & Batas Stok" subtitle="Satuan terkecil yang dijadikan dasar mutasi inventori.">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label for="purchase_price" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Harga Beli Acuan (HPP Dasar) *</label>
                        <div class="relative">
                    ...
                    <div class="flex items-center gap-2 pt-6">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ $product->is_active ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
            ...
            <x-card title="3. Satuan Kemasan Sekunder (Multi-Satuan / UOM)" subtitle="Konfigurasi penjualan satuan bertingkat (misal: 1 Dus = 40 Pcs, 1 Pak = 10 Pcs).">
                ...
                <div x-show="units.length === 0" class="py-8 text-center border-2 border-dashed border-slate-200 rounded-2xl">
                    ...
                    <button type="button" @click="addUnit()" class="mt-2 text-xs font-bold text-indigo-600 hover:text-indigo-800">
                        Klik di sini untuk menambah kemasan Dus/Pak
                    </button>
                </div>
